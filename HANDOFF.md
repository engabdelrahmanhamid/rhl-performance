...
## §16: إصلاح ثلاث مشاكل في شاشة تقييم المقيّم (2026-09-15)

صاحب المشروع اختبر شاشة التقييم الفعلية (`/evaluator/evaluate/[id]`, مكوّن `KpiCard.tsx`) وأبلغ عن 3 ملاحظات:

1. **🐛 خلل حقيقي (الأولوية)**: تقييمات "المبادرة" (مؤشر `SUBCRITERIA_RATING`، معايير فرعية مثل "تحمل المسؤولية") **لا تُحفظ إطلاقًا** رغم تكرار المحاولة. **السبب الجذري**: `handleSubScoreChange` في `KpiCard.tsx` كان يحدّث الحالة المحلية (React state) فقط، ثم يتحقق `if (!just.trim()) return;` قبل استدعاء `saveSubcriterionScore` - أي أن الضغط على رقم تقييم (قبل كتابة أي تبرير، وهو الترتيب الطبيعي في الواجهة: أزرار الرقم فوق مربع النص) كان يُسقِط الحفظ **صامتًا تمامًا** بلا أي خطأ ظاهر. تم التحقق من الإصلاح فعليًا (ليس افتراضًا) عبر بيانات اختبار محلية: تقييم فرعي بلا تبرير أصبح يُحفَظ في قاعدة البيانات فورًا عند الضغط.
2. **التبرير إلزامي فقط للتقييم المنخفض**: كان التبرير يُعرَض/يُطلَب لأي قيمة تقييم (1 إلى 5) لكل من المؤشر الرئيسي (`RATING_1_5`) وكل معيار فرعي - غيّرته ليصبح إلزاميًا فقط عند تقييم **أقل من 3** (تحقّقت فعليًا: عند 3 فأكثر لا يظهر أي طلب تبرير). العتبة (<3) قرار افتراضي مني - قابل للتعديل لو صاحب المشروع يريد عتبة مختلفة (مثلًا ≤2 فقط). طُبِّق التعديل في مكانين: `KpiCard.tsx` (المؤشر المرئي في الواجهة) و`validateReviewEvaluatorForSubmit` في `src/lib/services/reviewEngine.ts` (التحقق الفعلي وقت إرسال التقييم النهائي - هذا هو مكان الفرض الحقيقي، وليس الحفظ التلقائي لكل حقل الذي يبقى بلا حجب أبدًا لتفادي تكرار نفس خلل رقم 1).
3. **حقل نسبة مئوية بشريط تمرير**: أُضيف `<input type="range">` (0-100) بجانب حقل الرقم لأي مؤشر `PERCENTAGE` تحديدًا (مؤشرات `NUMBER`/`CURRENCY` بقيت بحقل رقم عادي فقط). تحقّقت فعليًا أن كتابة رقم في الحقل تحرّك الشريط، وأن القيمة تُحفَظ في `ReviewKpiActual.actualValue` بنفس الآلية السابقة.

كل الثلاثة اختُبرت **فعليًا محليًا** (بيانات اختبار مؤقتة أُنشئت وحُذفت بعد التحقق - لم تُحفَظ في الريبو) عبر تسجيل دخول كمقيّم حقيقي والتفاعل مع الشاشة، وليس بالقراءة النظرية للكود فقط. `npx tsc --noEmit` و68/68 اختبار ناجحان.
